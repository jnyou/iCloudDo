<template>
  <div class="app-container">
    ...
    <el-form-item>
        <el-select v-model="searchObj.status" clearable placeholder="课程状态">
...
export default {
  ...
  methods: {
    fetchData(page = 1) { // 调用api层获取数据库中的数据
      ...
      courseApi.pageCondition(this.page, this.limit, this.searchObj).then(response => {
        // debugger 设置断点调试
        if (response.success === true) {
          this.list = response.data.date
    ...
    // 所有一级分类
    initSubjectList() {
      subjectApi.getSujectList().then(response => {
