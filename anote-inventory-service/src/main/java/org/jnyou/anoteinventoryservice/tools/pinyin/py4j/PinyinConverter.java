package org.jnyou.anoteinventoryservice.tools.pinyin.py4j;
...
import org.jnyou.anoteinventoryservice.tools.pinyin.py4j.exception.IllegalPinyinException;
import org.jnyou.anoteinventoryservice.tools.pinyin.py4j.util.ArrayUtils;
import org.jnyou.anoteinventoryservice.tools.pinyin.py4j.voc.Py4jDictionary;
	...
	public String getPinyin(String chinese) throws IllegalPinyinException {
		...
		for (int i = 0; i < chs.length; i++) {
			...
			if (py_arr == null || py_arr.length < 1) {
				System.out.println("pinyin array is empty, char:" + chs[i] + ",chinese:" + chinese);
				continue;
			}
			if (py_arr.length == 1) {
				...
				for (String py : py_arr) {
					...
					if (i >= 1 && i + 1 <= chinese.length()) {
						...
						if (duoYinZiMap.containsKey(py)
								&& duoYinZiMap.get(py).contains(left)) {
							resultPy = py;
							break;
						}
					}
					...
					if (i <= chinese.length() - 2) {
						...
						if (duoYinZiMap.containsKey(py)
								&& duoYinZiMap.get(py).contains(right)) {
							resultPy = py;
							break;
						}
					}
					...
					if (i >= 1 && i + 2 <= chinese.length()) {
						...
						if (duoYinZiMap.containsKey(py)
								&& duoYinZiMap.get(py).contains(middle)) {
							resultPy = py;
							break;
						}
					}
					...
					if (i >= 2 && i + 1 <= chinese.length()) {
						...
						if (duoYinZiMap.containsKey(py)
								&& duoYinZiMap.get(py).contains(left3)) {
							resultPy = py;
							break;
						}
					}
					...
					if (i <= chinese.length() - 3) {
						...
						if (duoYinZiMap.containsKey(py)
								&& duoYinZiMap.get(py).contains(right3)) {
							resultPy = py;
							break;
						}
					}
					...
					if (duoYinZiMap.containsKey(py)
							&& duoYinZiMap.get(py).contains(
									String.valueOf(chs[i]))) { // 默认拼音
						defaultPy = py;
					}
				}
