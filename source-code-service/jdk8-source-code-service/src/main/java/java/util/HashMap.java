...
public class HashMap<K, V> extends AbstractMap<K, V>
     ...
     * <p>
     * 时时刻刻要记住HashMap的容量(桶的个数)永远是2的整数次幂。初始容量16，每次扩容之后的容量都是前一次容量的两倍。
     * 比如当前容量是16扩容一次编程32，再扩容一次变成64。
     * <p>
     * 而且如果我们在new HashMap的时候，给了初始容量，但是给定的容量不是2的整数次幂，
     * 构造函数内部也会调用tableSizeFor()函数转换成2的整数次幂的。比如：传递3会转换成4，传递13会转换成16。
     * <p>
     ...
     *
     *
     * 一定要时刻记住HashMap的容量(数组的大小，我们也说桶的个数)永远都是2的整数次幂，扩容就是把容量扩大一倍。
     * 扩容之后的容量=原来容量*2。（桶的个数翻倍）就算你new HashMap()的时候给定的容量不是2的整数次幂。
     * HashMap内部也是会通过tableSizeFor()函数把容量转换成2的整数次幂。
     *
     * HashMap确定元素属于哪个桶是通过对该元素的key取hash之后再 & （容量-1）。这样就找到了桶的位置（其实是数组的下标）。
     *
     * 扩容前后桶的关系也要特别注意，扩容前属于同一个桶(桶的索引位置相同)里面的元素，在扩容之后只会有两个桶来放他们：
     * 要不还保留扩容前的桶的索引位置，要不就是通过扩容前的索引位置+扩容前的容量得和值确定位置。
     * 我们举个例子，假设原来的容量是16，那么扩容之后的容量就是32。
     * 假设原来桶的位置为index。那么这个桶里面的元素只会去到扩容之后桶的index位置，或者桶的index+16位置。
     * 为什么会有这样的规律，关键在容量永远都是2的整数次幂。而且扩容是*2的扩容。为了加深大家的理解，我用一个图例来说明。
     *
     * @return the table
     */
    /**
     ...
     *
     * HashMap的树化，就是把链表转换为红黑树。
     * 当往HashMap里面添加元素的时候，随着桶里面元素的增加，当桶里面元素的个数大于8（TREEIFY_THRESHOLD），
     * 并且HashMap的容量大于64（MIN_TREEIFY_CAPACITY）的时候才会把链表树化成红黑树。
     ...
     *
     * 5、每个红色节点的两个子节点都是黑色。（从每个叶子节点到根节点的所有路径上不能有两个连续的红色节点）
     *
            ...
            */
            for (TreeNode<K,V> xp, xpp, xppl, xppr;;) {
                ...
                if (xp == (xppl = xpp.left)) { // 父节点是爷爷节点的左孩子
                    if ((xppr = xpp.right) != null && xppr.red) { // 如果右叔叔不为空 并且 为红色
