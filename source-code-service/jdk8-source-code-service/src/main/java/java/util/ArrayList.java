...
public class ArrayList<E> extends AbstractList<E>
     ...
     * 初始化默认的容量大小，如果实例化时未指定容量，则在初次添加元素时会进行扩容使用此容量作为数组长度
     */
     ...
     * ArrayList中的元素个数：比如添加了一个元素，大小就为默认的10
     */
    private int size;
     ...
     * if c.toArray成object[]失败用Arrays.copyOf将elementData存储数据转换成object[]对象拷贝到elementData中
     */
     ...
     * 把ArrayList数组中的空位置给减掉了（修剪ArrayList的大小）
     */
    ...
    /**
     * 操作数++，操作数用来多线程中一个线程正在操作ArrayList，而另一个也进行操作完成,导致该线程操作数变化从而引起报错
    ...
    private void grow(int minCapacity) {
        ...
        // minCapacity is usually close to size, so this is a win: 将新数组拷贝到elementData
     ...
     */
    /**
     * 想要提升ArrayList的大批量插入速度，我们可以减少扩容操作，给一个大的初始值进行创建ArrayList，
     * 这样做能够使得ArrayList的大批量插入元素速度变得非常快。
     *
     * @param index
     * @param element
     * @Author jnyou
    ...
    public void add(int index, E element) {
        ...
        // 判断是否需要扩容,记录操作数
        ensureCapacityInternal(size + 1);  // Increments modCount!!
        // 将elementData中从index开始的元素向后移动一个位置
        System.arraycopy(elementData, index, elementData, index + 1,
                size - index);
        // index位置的元素赋值为指定element （在index位置添加元素）
        elementData[index] = element;
        size++;
    }
    ...
    public E remove(int index) {
        ...
        //需要移动的长度，如果==0就不必拷贝移动了。ps：size = 1而index为0，那么删除一个元素之后为空，就不必arraycopy
    ...
    private boolean batchRemove(Collection<?> c, boolean complement) {
        ...
        try {
            for (; r < size; r++)
                // 对elementData中的每个元素进行判断
                // 如果指定的集合c包含了elementData中的元素且等于true，留下该元素，retainAll使用
                // 如果指定的集合c包含了elementData中的元素且等于false，删除该元素，removeAll使用
                if (c.contains(elementData[r]) == complement)
                    elementData[w++] = elementData[r];
        } finally {
            // Preserve behavioral compatibility with AbstractCollection,
            // even if c.contains() throws.
            if (r != size) { // 还有剩余元素，将这些元素拷贝到从w开始的位置
                System.arraycopy(elementData, r,
                        elementData, w,
                        size - r);
                w += size - r;
            }
            if (w != size) { // 将从w开始的位置都置空
                // clear to let GC do its work
    ...
    private class Itr implements Iterator<E> {
        ...
        @SuppressWarnings("unchecked")
        public E next() {
            // 快速失败，检查expectedModCount和modCount是否相等，不相等直接抛出错误
